...
private:
  ...
  mlir::Value genProcedureRef(const Fortran::evaluate::ProcedureRef &procRef,
    ...
    Fortran::lower::CallerInterface caller(procRef);
    auto func = caller.getFuncOp();
    llvm::SmallVector<mlir::Value, 3> operands(caller.getNumFIRArguments());

    // helper to embox and place arguments pass in memory.
    auto emboxAndPlaceArgument = [&](const Fortran::lower::PassInfo &arg,
                                     mlir::Value argRef) {
      if (arg.passBy == Fortran::lower::PassEntityBy::BaseAddress) {
        arg.placeArgument(argRef, operands);
      } else if (arg.passBy == Fortran::lower::PassEntityBy::BoxChar) {
        auto boxChar = argRef;
        ...
        arg.placeArgument(boxChar, operands);
      } else if (arg.passBy == Fortran::lower::PassEntityBy::Descriptor) {
        TODO();
      } else if (arg.passBy == Fortran::lower::PassEntityBy::AddressAndLength) {
        auto ch = builder.materializeCharacter(argRef);
        arg.placeAddressAndLength(ch.first, ch.second, operands);
      } else {
        llvm_unreachable("pass by value not handled here");
      }
    };

    for (const auto &arg : caller.getPassedActuals()) {
      if (!arg.actual.has_value())
        TODO(); // optional arguments
      const auto *expr = arg.actual->UnwrapExpr();
      if (!expr)
      ...
      if (arg.passBy == Fortran::lower::PassEntityBy::Value) {
        if (!argVal)
          argVal = genLoad(argRef);
        arg.placeArgument(argVal, operands);
        continue;
      }
      ...
      emboxAndPlaceArgument(arg, argRef);
    }

    // Handle case where caller must pass result
    mlir::Value resRef;
    if (auto resultArg = caller.getPassResult()) {
      resRef = builder.createTemporary(getLoc(), resultType[0]);
      emboxAndPlaceArgument(*resultArg, resRef);
    }
    ...
    if (caller.getPassResult())
      return resRef; // FIXME: probably needs packing
